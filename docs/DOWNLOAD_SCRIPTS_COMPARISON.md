# download_doc vs download_wiki 详细对比
...
| 维度 | download_doc | download_wiki | 胜出 |
|------|-------------|---------------|------|
| **操作对象** | 单个文档 | Wiki空间/目录 | - |
| **输出形式** | 单个Markdown文件 | Markdown文件目录 | - |
| **操作粒度** | 精确定位 | 批量处理 | - |
| **递归搜索** | ✅ 支持（-n参数） | ❌ 不支持 | download_doc |
| **深度控制** | ❌ 不支持 | ✅ 支持（-d参数） | download_wiki |
| **目录结构** | ❌ 不保持 | ✅ 自动保持 | download_wiki |
| **交互式选择** | ✅ 支持 | ❌ 不支持 | download_doc |
| **文件名控制** | ✅ 精确控制 | ⚠️ 自动生成 | download_doc |
| **批量处理** | ❌ 单个 | ✅ 批量 | download_wiki |
| **部分下载** | ❌ 不支持 | ✅ 支持（-S参数） | download_wiki |
...
两个脚本实际上是**互补关系**，而非竞争关系：

### 功能互补矩阵

| 功能 | download_doc | download_wiki | 建议 |
|------|-------------|---------------|------|
