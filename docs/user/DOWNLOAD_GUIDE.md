...
`--doc-name` 和 `--wiki-path` 的性能差异：

**根级别文档**（如 `/文档名` 或 `文档名`）：
```bash
# 修复后：快（~0.5秒）- 推荐
uv run python scripts/download_doc.py -s "个人知识库" -n "表格测试-220326" -o test.md
```

**子文件夹文档**（如 `/文件夹/文档名`）：
```bash
# 必须使用 --wiki-path（不支持递归搜索）
uv run python scripts/download_doc.py -s "个人知识库" -p "/文件夹/表格测试-220326" -o test.md
```

| 文档位置 | 使用 `--doc-name` | 使用 `--wiki-path` |
|----------|------------------|-------------------|
| 根级别 | ✅ 快速（~0.5s） | ✅ 快速 |
| 子文件夹 | ❌ 不支持 | ✅ 快速 |

**使用建议**：
- 根级别文档 → 使用 `--doc-name`（快速）
- 子文件夹文档 → 使用 `--wiki-path`（指定完整路径）
